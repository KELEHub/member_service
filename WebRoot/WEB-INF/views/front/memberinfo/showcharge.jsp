			...
			<div class="widget-box">
				...
				<form action="/acc/doacccharge.do" method="POST"
					...
					<div class="control-group">
						...
						<div class="controls">
						   <textarea rows="5" cols="500" style="width: 600px;" disabled="disabled" name="chageBackInfo">姓名: ${result.name} > 银行名称:${result.bankName} > 银行账号:${result.bankCard} > 银行地址:${result.bankProvince}${result.bankCity}${result.bankCounty}${result.bankAddress} > 联系电话:${result.phoneNumber}</textarea>
						</div>
					</div>
					<div class="control-group">
					...
					<div class="control-group">
						...
						<div class="controls">
						 <textarea rows="4" cols="500" style="width: 600px;" name="chageMessage"></textarea>
						</div>
					</div>
					<div class="control-group">
						...
						<tbody>
							<c:forEach var="item" items="${charList }">
								<tr class="gradeX">
									...
									<td><button class="btn" onclick="showAccChargeDetail('${item.id}')">备注信息</button></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
